...
public class VndService : IVndService
{
    ...
    */
        ...
        VndDocument vnd, VndRedaction redaction, EditLastRevisionDirectlyRequest request, int currentUserId)
    {
        ...
        // Право EditLastRevisionDirectly (главный редактор/администратор) даёт безусловное
        // редактирование ЛЮБОЙ незавершающейся-согласованием редакции — в т.ч. уже
        // действующей (Approved/NotRequired), см. комментарий выше. Без этого права редактировать
        // можно только СВОЙ ещё не согласованный черновик (ApprovalStatus.Draft) — то же самое
        // население пользователей, которому и так разрешено грузить новую редакцию/запускать
        // согласование (IsChiefEditor/IsLinkedToVndAsync). Это единственный способ для обычного
        // инициатора исправить содержимое редакции, отклонённой согласующим при первичном/
        // повторном согласовании (см. VndApprovalService.RejectApprovalAsync — она возвращает
        // ApprovalStatus в Draft, но AddRedactionAsync не даст загрузить взамен новую редакцию,
        // пока эта не "закрыта" — см. проверку там же), иначе процесс зацикливается: без права
        // редактировать содержимое, единственное доступное действие — отправить на согласование
        // тот же самый, ничем не отличающийся файл.
        if (!_currentUser.HasPermission(PermissionCode.EditLastRevisionDirectly))
        {
            if (redaction.ApprovalStatus != RedactionApprovalStatus.Draft)
                throw new UnauthorizedAccessException(
                    "Редактировать эту редакцию напрямую может только главный редактор ВНД");

            if (!IsChiefEditor() && !await IsLinkedToVndAsync(vnd, currentUserId))
                throw new UnauthorizedAccessException(
                    "Редактировать черновик редакции может только разработчик, куратор, ответственный " +
                    "исполнитель, ответственный за актуализацию или главный редактор ВНД");
        }
