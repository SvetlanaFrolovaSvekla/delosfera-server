...
public class OrganizationUnitService : IOrganizationUnitService
{
        ...
        string languageCode)
    {
        ...
        // Подразделение пришло из портала — значит портал распоряжается его названием
        // и местом в структуре: их он перезаписывает при каждом проходе, и правка
        // здесь дожила бы лишь до ближайшей ночи. Молча принять её было бы хуже отказа.
        //
        // Начальника и куратора портал присылает не для всех узлов (у многих их нет),
        // а маршруты кадровых СЗ без них не собираются (КСЗ-11). Поэтому их правка
        // разрешена: синхронизация перезапишет значение только там, где портал сам
        // прислал руководителя, — пустые узлы, заполненные вручную, она не трогает
        // (см. OrgSyncService.LinkUnitHeadsAsync: пустое не затирает проставленного).
        if (entity.ExternalId is not null)
        {
            ...
            if (занято.Count > 0)
                throw new InvalidOperationException(
                    ...
                    "Меняйте в портале; здесь можно править переводы названия, начальника и куратора.");
        }
