...
public class AttachmentsTests
{
    ...
    private static ILetterService Письма(DelosferaDbContext db, int userId) =>
        new LetterService(db, new FakeCurrentUser(userId), new ХранилищеВПамяти(db), new delosfera_server.Modules.Documents.Services.AuditService(db), new delosfera_server.Modules.Documents.Services.NumeratorService(db), new delosfera_server.Common.Services.BankClock());

    private static IPoaService Доверенности(DelosferaDbContext db) =>
        new PoaService(db, new ХранилищеВПамяти(db), new delosfera_server.Modules.Documents.Services.AuditService(db), new delosfera_server.Modules.Documents.Services.NumeratorService(db), new delosfera_server.Common.Services.BankClock());
